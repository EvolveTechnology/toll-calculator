...
public class TollCalculator {
	...
	public int getTollFee(Vehicle vehicle, Date... dates) {
		...
		// Definition of 1 day based on calendar day not in 24 hours
		if(dates.length > 1) {
			...
			if(startYear != endYear || startMonth != endMonth || startDay != endDay) {
				throw new RuntimeException("Invalid date ranges: only one day of times supported");
			}
		...
		for (Date date : dates) {
			...
			if (totalFee > 60) { // No need to calculate further 
				totalFee = 60; 
				return totalFee;
			}
			
			int currentFee = getTollFee(date, vehicle);
			int prevFee = getTollFee(intervalStart, vehicle);

			TimeUnit timeUnit = TimeUnit.MINUTES;
			long diffInMillies = date.getTime() - intervalStart.getTime();
			long minutes = timeUnit.convert(diffInMillies, TimeUnit.MILLISECONDS);

			if (counter > 0 && minutes <= 60) {
				if (totalFee > 0)
					totalFee -= prevFee;
